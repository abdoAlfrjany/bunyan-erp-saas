...
export async function PATCH(req: NextRequest) {
  try {
    ...
    // 1. جلب الطلبية الحالية
    const { data: order, error: fetchError } = await supabaseAdmin
      .from('orders')
      .select('id, tenant_id, status, total, delivery_type, items, order_number, courier_package_id, courier_company_id, prepaid_amount')
      .eq('id', orderId)
      .single();
    ...
    // 3. تأثيرات الخزينة عند التسليم (فقط للداخلية)
    if (isInternal && status === 'delivered' && prevStatus !== 'delivered') {
      ...
      if (cashAccount) {
        const { error: rpcError } = await supabaseAdmin.rpc('create_treasury_transaction_atomic', {
          ...
          p_amount: Math.abs(order.total),
          p_description: `إيراد طلبية ${order.order_number}`,
    ...
    // 4. تأثيرات الإلغاء / المرتجع عبر الدالة المشتركة
    if (['cancelled', 'return_confirmed'].includes(status)) {
      await applyCancelSideEffects(supabaseAdmin, { ...order, status: prevStatus }, {
        newStatus: status as 'cancelled' | 'return_confirmed',
        userId: auth.userId,
        cancelVanexShipment: status === 'cancelled', // إلغاء فقط وليس مرتجع
      });
    }
