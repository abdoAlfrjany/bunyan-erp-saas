...
export default function OrdersPage() {
  ...
  const { orders, products, couriers, getForTenant, addOrder, updateOrderStatus } = useDataStore();
  ...
  const [confirmAction, setConfirmAction] = useState<{ id: string; status: Order['status'], label: string } | null>(null);
  ...
  const orderCalculations = useMemo(() => {
    ...
    const deliveryFee = newOrder.deliveryType === 'courier_company' && courier ? courier.defaultDeliveryFee : 0;
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
                  ...
                  <tr key={o.id} className="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-1.5 flex-wrap justify-center">
                        ...
                        {actions.length === 0 && <span className="text-xs text-gray-400">لا يوجد إجراء</span>}
                      </div>
                    </td>
                  </tr>
                );
      ...
      <SlideOver isOpen={slideOpen} onClose={() => setSlideOpen(false)} title="إنشاء طلبية جديدة" width="w-full sm:max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full pb-20">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4 border border-gray-200 rounded-xl p-4 bg-gray-50 h-[calc(100vh-140px)] overflow-y-auto">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="بحث سريع في المنتجات..." 
            ...
            <div className="space-y-2 mt-2">
              {myProducts
                ...
                .map(p => (
                  <div key={p.id} className="bg-white p-3 border border-gray-100 rounded-xl flex items-center justify-between shadow-sm hover:border-bunyan-300 transition-colors">
                    <div>
                      <p className="text-sm font-bold text-gray-900">{p.name}</p>
                      <p className="text-xs text-gray-500 font-mono mt-0.5">BN{p.itemCode} • متاح {p.quantity}</p>
                    </div>
                    <div className="flex flex-col items-end">
                      <p className="text-sm font-bold font-currency text-bunyan-600 mb-2">{formatCurrency(p.sellingPrice)}</p>
          ...
          <div className="lg:col-span-7 flex flex-col gap-6 h-[calc(100vh-140px)] overflow-y-auto pr-2 pb-10">
            ...
            <div className="border border-gray-200 rounded-xl p-4 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="bg-bunyan-100 text-bunyan-800 text-xs font-bold px-2 py-1 rounded-full">{orderItems.length} عناصر</span>
              </div>
            ...
            <div className="border border-gray-200 rounded-xl p-4 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={newOrder.deliveryType} onChange={(e) => setNewOrder({ ...newOrder, deliveryType: e.target.value as any, courierId: '' })}
                ...
                {newOrder.deliveryType === 'courier_company' && newOrder.courierId && (
                  <div className="sm:col-span-2">
                    ...
                    <select value={newOrder.customerCity} onChange={(e) => setNewOrder({ ...newOrder, customerCity: e.target.value })}
                      className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:border-bunyan-500">
                      <option value="">اختر مدينة الشحن المعتمدة...</option>
                      {(myCouriers.find(c => c.id === newOrder.courierId)?.cities || []).map(city => (
            ...
            <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-5 border border-gray-200 shadow-sm">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 font-medium">مجموع المنتجات فرعي</span>
                  <span className="font-bold font-mono text-gray-900">{formatCurrency(orderCalculations.subtotal)}</span>
                </div>
                <div className="flex justify-between text-sm items-center">
                  ...
                  <div className="w-24 relative">
                    <input type="number" min={0} value={newOrder.discount || ''} onChange={(e) => setNewOrder({ ...newOrder, discount: Number(e.target.value) })}
                      className="w-full px-2 py-1.5 border border-gray-300 rounded-lg text-sm text-left focus:outline-none focus:border-bunyan-500 focus:ring-2 focus:ring-bunyan-500/20 font-mono text-red-600 font-bold bg-white" placeholder="0 د.ل" />
                  </div>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 font-medium pb-1 w-full flex justify-between border-b border-dashed border-gray-300">
                    <span>رسوم التوصيل {newOrder.priceIncludesDelivery && <span className="text-green-600 text-xs mr-2">(مشمولة مسبقاً)</span>}</span>
                    <span className="font-mono text-gray-900">{newOrder.priceIncludesDelivery ? '0 د.ل' : formatCurrency(orderCalculations.deliveryFee)}</span>
                  </span>
                </div>
              </div>
              <div className="flex justify-between text-xl font-black pt-4 border-t border-gray-300/50 mt-4">
                ...
                <span className="text-bunyan-700 font-mono">{formatCurrency(orderCalculations.total)}</span>
              </div>
            </div>
          </div>
      ...
      <ConfirmDialog
        ...
        message={`هل أنت متأكد من تغيير حالة الطلبية "${filtered.find((o) => o.id === confirmAction?.id)?.orderNumber || ''}"؟${
          confirmAction?.status === 'cancelled' ? ' (سيتم إرجاع المخزون وتسوية الحسابات الخاصة بالفاتورة)' :
          confirmAction?.status === 'return_confirmed' ? ' (سيتم إرجاع المخزون وتسوية مستحقات التوصيل إن وجدت)' :
          confirmAction?.status === 'delivered' ? ' (سيتم إغلاق الفاتورة وتسجيل المبيعات)' : ''
        }`}
        variant={confirmAction?.status === 'cancelled' || confirmAction?.status === 'return_confirmed' ? 'danger' : 'primary'}
      />
    </div>
  );
}
